...
class GapFinding:
    def __init__(self):
        ...
        self.angle_max = math.radians(rospy.get_param('~angle_max', 80.0))
        self.angle_min = math.radians(rospy.get_param('~angle_min', -80.0))
        self.angle_initialize_flag = False
        self.lookahead_range = 3.0
        self.cluster_density = 3.0
        self.skip_points = 3
        ...
        for dist in range_data:
            if self.angle_min <= current_angle <= self.angle_max:
                ...
                if dist <= self.lookahead_range:
                    ...
                    gap_distance = self.distance(self.toXYZ(dist, current_angle), self.toXYZ(last_radius, last_angle))
